...
int main(int argc, char *argv[]) {
  ...
  hist = cv::Mat::zeros( cv::Size( histsize, histsize ), CV_32FC1 ); // CV_32FC1: A single-channel matrix (or image) where each pixel or element is a 32-bit float.
  ...
  for( int i=0;i<src.rows;i++) {
    ...
    for(int j=0;j<src.cols;j++) {
      ...
      int rindex = (int)( r * (histsize - 1) + 0.5 ); // normalize the range to [0, histrsize - 1], 0.5 is to round a floating-point num to represnet the nearest integer when casting to an integer type
  ...
  for(int i=0;i<hist.rows;i++) {
    ...
    for(int j=0;j<hist.cols;j++) {
      if( i + j > hist.rows ) { // r + g <= 1 to form a valid trangular histogram
